(function($, und) {
	$(function() {
		$('.bitcoin-button').each(function () {
			...
			wrapper.append('<a href="' + url + '" class="bitcoin-counter" style="display:none;">&nbsp;</a>');
			wrapper.append('<div class="bitcoin-bubble" style="display:none;"><a href="' + url + '"><img src="http://chart.googleapis.com/chart?chs=200x200&cht=qr&chld=H|0&chl=' + encodeURIComponent(url) + '" class="qr-code" width="200" height="200" alt="" /><span class="address">' + address + '</span></div>');
			...
			if ( show_info && show_info != 'none' ) {
				$.get(bitcoin_button.url, { action: 'bitcoin-address-info', address: address }, function (data) {
				    var counter = wrapper.find('.bitcoin-counter');
